...
var DashboardLoader = /** @class */ (function () {
    function DashboardLoader(config) {
        ...
        this.loadParts = function (ctx, dashboardWithPlugins, config, beforeExternalPluginLoaded) {
            if (config === void 0) { config = _this.config; }
            return __awaiter(_this, void 0, void 0, function () {
                var engineLoader, pluginLoader, beforeLoad, _a, engine, plugins, additionalPlugins, loadedPlugins, areLoadedPluginsCompatibleWithDashboardEngine, allPlugins;
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        ...
                        case 5:
                            ...
                            areLoadedPluginsCompatibleWithDashboardEngine = loadedPlugins.every(function (p) {
                                return isPluginCompatibleWithDashboardEngine(engine, p);
                            });
                            ...
                            allPlugins = areLoadedPluginsCompatibleWithDashboardEngine
                                ? __spreadArrays(loadedPlugins, additionalPlugins) : additionalPlugins;
                            return [2 /*return*/, [engine, allPlugins]];
                    }
                });
            });
        };
        this.load = function (options) { return __awaiter(_this, void 0, void 0, function () {
            var _a, backend, dashboard, filterContextRef, dashboardRef, _b, workspace, clientWorkspace, dashboardWithPlugins, _c, ctx, pluginsAreValid, _d, externalPluginLoaded, beforeExternalPluginLoaded, _e, engine, plugins, extensionProps, config, dashboardConfig, props, DashboardComponent;
            return __generator(this, function (_f) {
                switch (_f.label) {
                    case 0:
                        _a = this.baseProps, backend = _a.backend, dashboard = _a.dashboard, filterContextRef = _a.filterContextRef;
                        dashboardRef = typeof dashboard === "string" ? idRef(dashboard) : dashboard;
                    ...
                    case 1:
                        _b = _f.sent(), workspace = _b[0], clientWorkspace = _b[1];
                    ...
                    case 5:
                        ...
                        externalPluginLoaded = false;
                        beforeExternalPluginLoaded = function (params) {
                            if (params.externalPluginsCount > 0) {
                                externalPluginLoaded = true;
                            }
                        };
                        return [4 /*yield*/, this.loadParts(ctx, dashboardWithPlugins, !pluginsAreValid ? StaticLoadStrategies : this.config, beforeExternalPluginLoaded)];
                    case 6:
                        _e = _f.sent(), engine = _e[0], plugins = _e[1];
                        extensionProps = engine.initializePlugins(ctx, plugins);
                        config = this.baseProps.config;
                        dashboardConfig = config;
                        if ((options === null || options === void 0 ? void 0 : options.allowUnfinishedFeatures) === "staticOnly" && externalPluginLoaded) {
                            dashboardConfig = __assign(__assign({}, config), { allowUnfinishedFeatures: false });
                        }
                        props = __assign(__assign(__assign(__assign({}, this.baseProps), { config: dashboardConfig }), extensionProps), { workspace: workspace, dashboard: dashboardWithPlugins === null || dashboardWithPlugins === void 0 ? void 0 : dashboardWithPlugins.dashboard });
