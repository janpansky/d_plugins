...
var DATE_FORMAT_REGEX_TIME_SUPPORT = /^\d{4}-\d{2}-\d{2}( \d{2}:\d{2})?$/;
...
export function isValidDateFilterFormat(filterItem, shouldValidateDataSet, isTimeSupported) {
    if (shouldValidateDataSet === void 0) { shouldValidateDataSet = true; }
    if (isTimeSupported === void 0) { isTimeSupported = false; }
    if (EmbeddedGdc.isAbsoluteDateFilter(filterItem)) {
        ...
        var valueFormatRegex = isTimeSupported ? DATE_FORMAT_REGEX_TIME_SUPPORT : DATE_FORMAT_REGEX;
...
export function isValidFilterItemFormat(filterItem, shouldValidateDataSet, isTimeSupported) {
    if (shouldValidateDataSet === void 0) { shouldValidateDataSet = true; }
    if (isTimeSupported === void 0) { isTimeSupported = false; }
...
export function isValidFiltersFormat(filters, shouldValidateDataSet, isTimeSupported) {
    if (shouldValidateDataSet === void 0) { shouldValidateDataSet = true; }
    if (isTimeSupported === void 0) { isTimeSupported = false; }
    return (!isEmpty(filters) &&
        filters.every(function (filter) { return isValidFilterItemFormat(filter, shouldValidateDataSet, isTimeSupported); }));
}
