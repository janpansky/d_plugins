...
export function getItemsFromBuckets(buckets, localIdentifiers, types) {
    return localIdentifiers.reduce(function (bucketItems, localIdentifier) {
        ...
        bucketItems.push.apply(bucketItems, toAdd);
        return bucketItems;
    }, []);
}
...
export function getAllItemsByType(buckets, types) {
    return buckets.reduce(function (items, bucket) {
        items.push.apply(items, getAllBucketItemsByType(bucket, types));
        return items;
    }, []);
}
...
export function getMeasureItems(buckets) {
    ...
    var preferredMeasures = preference.reduce(function (acc, pref) {
        ...
        acc.push.apply(acc, prefBucketItems);
        return acc;
    }, []);
...
export function getAllAttributeItemsWithPreference(buckets, preference) {
    var preferredAttributes = preference.reduce(function (acc, pref) {
        ...
        if ((_a = prefBucket === null || prefBucket === void 0 ? void 0 : prefBucket.items) === null || _a === void 0 ? void 0 : _a.length) {
            acc.push.apply(acc, prefBucket.items);
        }
        return acc;
    }, []);
    ...
    var allOtherAttributes = otherBucketNames.reduce(function (attributes, bucketName) {
        attributes.push.apply(attributes, getBucketItemsByType(buckets, bucketName, [ATTRIBUTE, DATE]));
        return attributes;
    }, []);
...
export var isComparisonAvailable = function (buckets, filters) {
    var itemsFromBucket = buckets.reduce(function (acc, bucket) {
        acc.push.apply(acc, bucket.items);
        return acc;
    }, []);
