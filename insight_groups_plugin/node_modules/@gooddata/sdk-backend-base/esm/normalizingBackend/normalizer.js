...
import { attributeAlias, attributeLocalId, filterIsEmpty, isArithmeticMeasureDefinition, isAttributeLocator, isAttributeSort, isLocalIdRef, isMeasureSort, isMeasureValueFilter, isNegativeAttributeFilter, isPoPMeasureDefinition, isPreviousPeriodMeasureDefinition, measureAlias, measureFormat, MeasureGroupIdentifier, measureLocalId, measureMasterIdentifier, measureTitle, measureValueFilterCondition, modifyAttribute, modifyMeasure, isAttributeFilter, isRankingFilter, isAttributeDescriptor, isMeasureDescriptor, isMeasureGroupDescriptor, isResultMeasureHeader, isMeasureDefinition, } from "@gooddata/sdk-model";
...
var Denormalizer = /** @class */ (function () {
    function Denormalizer(state) {
        ...
        this.denormalizeDimDescriptors = function (normalizedDims) {
            var _a;
            var measureGroup = (_a = normalizedDims
                .find(function (normalizedDim) { return normalizedDim.headers.find(function (header) { return isMeasureGroupDescriptor(header); }); })) === null || _a === void 0 ? void 0 : _a.headers.find(function (header) { return isMeasureGroupDescriptor(header); });
            return cloneDeepWith(normalizedDims, function (value) {
                ...
                if (isAttributeDescriptor(value)) {
                    ...
                    var masterMeasureId_1 = measureMasterIdentifier(measure);
                    var masterMeasureInheritFormat = isMeasureGroupDescriptor(measureGroup) && ((_a = measureGroup.measureGroupHeader.items.find(function (item) {
                        return _this.originalLocalId(item.measureHeaderItem.localIdentifier) === masterMeasureId_1;
                    })) === null || _a === void 0 ? void 0 : _a.measureHeaderItem.format);
                    /**
                    ...
                    var format = measureFormat(measure) ||
                        (masterMeasureId_1 && measureFormat(_this.originalMeasures[masterMeasureId_1])) ||
...
var Normalizer = /** @class */ (function () {
    function Normalizer(original, options) {
        var _this = this;
        ...
        this.normalizeAttributes = function () {
            _this.normalized.attributes.forEach(function (attr) {
                if (!_this.options.keepRemovableProperties) {
                    delete attr.attribute.alias;
                }
                var originalLocalId = attributeLocalId(attr);
                var normalizedLocalId = attributeLocalId(modifyAttribute(attr, function (m) { return m.noAlias().defaultLocalId(); }));
        ...
        this.normalizeMeasure = function (measure, path) {
            ...
            var newLocalId = measureLocalId(modifyMeasure(measure, function (m) { return m.noAlias().noTitle().defaultFormat().defaultLocalId(); }));
    ...
    Normalizer.normalize = function (def, options) {
        var n = new Normalizer(def, options);
        return n.normalize();
    };
    return Normalizer;
}());
