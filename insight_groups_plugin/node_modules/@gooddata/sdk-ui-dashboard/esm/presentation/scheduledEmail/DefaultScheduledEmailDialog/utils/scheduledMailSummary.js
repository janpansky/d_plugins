...
function getScheduledEmailRepeatString(intl, options, startDate) {
    var _a;
    var repeatType = options.repeatType, repeatPeriod = options.repeatPeriod, repeatFrequency = options.repeatFrequency;
    ...
    if (!isCustomRepeatType) {
        return intl.formatMessage(messages["scheduleDialogEmailRepeats_" + repeatType], { day: day, week: week });
    }
    ...
    var frequencies = intl.formatMessage(messages["scheduleDialogEmailRepeatsFrequencies_" + repeatFrequencyType], {
        n: repeatPeriod,
    });
    var appliedRepeatExecuteOn = repeatFrequencyType === REPEAT_FREQUENCIES.WEEK
...
export function getScheduledEmailSummaryString(intl, recurrency, startDate) {
    var repeatDays = getScheduledEmailRepeatString(intl, recurrency, startDate);
    var atLocalization = intl.formatMessage({ id: "gs.date.at" });
    var time = getFormattedTime(recurrency.time);
