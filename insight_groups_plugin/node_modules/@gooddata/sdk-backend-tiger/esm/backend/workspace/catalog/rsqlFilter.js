...
function parseTagsToRSQLFormat(tagsIdentifiers) {
    return tagsIdentifiers.map(function (tag) { return (tag.indexOf(" ") >= 0 ? "'" + tag + "'" : tag); });
}
export function tagsToRsqlFilter(_a) {
    ...
    if (!isEmpty(includeTags)) {
        ...
        var includeParsedTagsIdentifiers = parseTagsToRSQLFormat(includeTagsIdentifiers);
    ...
    if (!isEmpty(excludeTags)) {
        ...
        var excludeParsedTagsIdentifiers = parseTagsToRSQLFormat(excludeTagsIdentifiers);
