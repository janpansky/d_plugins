...
export var repeatItemsNTimes = function (array, n) {
    return new Array(n).fill(null).reduce(function (result) {
        result.push.apply(result, array);
        return result;
    }, []);
};
export var unEscapeAngleBrackets = function (str) { return str === null || str === void 0 ? void 0 : str.replace(/&lt;|&#60;/g, "<").replace(/&gt;|&#62;/g, ">"); };
 ...
 * @internal
 */
export var isFunnel = isEqual(VisualizationTypes.FUNNEL);
/**
 * @internal
 */
export var isSupportingJoinedAttributeAxisName = function (type) {
    return isBarChart(type) || isColumnChart(type) || isBulletChart(type);
};
/**
 * @internal
 */
...
var getNumberWithGivenDecimals = function (value, decimalNumbers) {
    return decimalNumbers === 0 ? Math.round(value) + "%" : parseFloat(value.toFixed(decimalNumbers)) + "%";
};
// it calculates number of decimals from default format that contains ',' and '.' as separators
var getNumberOfDecimalsFromDefaultFormat = function (format) {
    var splittedFormat = format.split(DEFAULT_DECIMAL_SEPARATOR);
    if (splittedFormat.length !== 2) {
        return 0;
    }
    return Array.from(splittedFormat[1]).reduce(function (numberOfDecimals, letter) { return (letter === "0" ? numberOfDecimals + 1 : numberOfDecimals); }, 0);
};
export function percentFormatter(value, format) {
    if (isNil(value)) {
        return "";
    }
    var isPercentageFormat = format && format.trim().slice(-1) === "%";
    var numberOfDecimals = isPercentageFormat ? getNumberOfDecimalsFromDefaultFormat(format) : 2;
