...
var COLUMN = VisualizationTypes.COLUMN, LINE = VisualizationTypes.LINE;
...
function isOnlyLineSeries(config, definition) {
    var _a = getDefaultComboTypes(config), primaryChartType = _a.primaryChartType, secondaryChartType = _a.secondaryChartType;
    var buckets = definition ? definition.buckets : [];
    var primaryBucket = bucketsFind(buckets, BucketNames.MEASURES);
    var secondaryBucket = bucketsFind(buckets, BucketNames.SECONDARY_MEASURES);
    var isEmptyPrimaryMeasure = !primaryBucket || bucketIsEmpty(primaryBucket);
    var isEmptySecondaryMeasure = !secondaryBucket || bucketIsEmpty(secondaryBucket);
    var isLineChartOnLeftAxis = isLineChart(primaryChartType);
    var isLineChartOnRightAxis = isLineChart(secondaryChartType);
...
export function getComboConfiguration(config, definition, theme) {
    ...
    var series = isOnlyLineSeries(config, definition)
        ? {
            series: {
                states: {
                    inactive: {
