...
import { bucketAttributes, bucketIsEmpty, bucketsFind, bucketTotals, MeasureGroupIdentifier, newBucket, newTwoDimensional, } from "@gooddata/sdk-model";
...
function pivotDimensions(def) {
    ...
    var rowAttributes = row ? bucketAttributes(row) : [];
    var columnAttributes = columns ? bucketAttributes(columns) : [];
    var measuresItemIdentifiers = measures && !bucketIsEmpty(measures) ? [MeasureGroupIdentifier] : [];
    var totals = row ? bucketTotals(row) : [];
    return newTwoDimensional(__spreadArrays(rowAttributes, totals), __spreadArrays(columnAttributes, measuresItemIdentifiers));
}
